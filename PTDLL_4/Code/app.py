...
def fetch_today_data():
    ...
    if current_time.hour < 5:  # Trước 5:00 AM GMT+7
        yesterday = date.today() - timedelta(days=1)
        message = f"Dữ liệu ngày hôm nay chưa chốt. Sử dụng dữ liệu ngày trước đó: {yesterday.strftime('%d/%m/%Y')}."
        result = messagebox.askyesno("Thông báo", message + "\nBạn có muốn sử dụng dữ liệu này không? (Có: Sử dụng, Không: Nhập thủ công)")
        if result:  # Nếu người dùng chọn "Có"
            try:
                amzn = yf.Ticker("AMZN").history(start=yesterday.strftime('%Y-%m-%d'), end=(yesterday + timedelta(days=1)).strftime('%Y-%m-%d'), interval="1d")
                sp500 = yf.Ticker("^GSPC").history(start=yesterday.strftime('%Y-%m-%d'), end=(yesterday + timedelta(days=1)).strftime('%Y-%m-%d'), interval="1d")
                oil = yf.Ticker("CL=F").history(start=yesterday.strftime('%Y-%m-%d'), end=(yesterday + timedelta(days=1)).strftime('%Y-%m-%d'), interval="1d")
                dollar_index = yf.Ticker("DX-Y.NYB").history(start=yesterday.strftime('%Y-%m-%d'), end=(yesterday + timedelta(days=1)).strftime('%Y-%m-%d'), interval="1d")

                if amzn.empty or sp500.empty or oil.empty or dollar_index.empty:
                    return None, "Không có dữ liệu cho ngày trước đó."
                ...
                return None, f"Lỗi khi lấy dữ liệu ngày trước đó: {str(e)}"
        else:
            return None, "Người dùng chọn nhập thủ công."
    else:
        today = date.today()

    try:
        amzn = yf.Ticker("AMZN").history(start=today.strftime('%Y-%m-%d'), end=(today + timedelta(days=1)).strftime('%Y-%m-%d'), interval="1d")
        sp500 = yf.Ticker("^GSPC").history(start=today.strftime('%Y-%m-%d'), end=(today + timedelta(days=1)).strftime('%Y-%m-%d'), interval="1d")
        oil = yf.Ticker("CL=F").history(start=today.strftime('%Y-%m-%d'), end=(today + timedelta(days=1)).strftime('%Y-%m-%d'), interval="1d")
        dollar_index = yf.Ticker("DX-Y.NYB").history(start=today.strftime('%Y-%m-%d'), end=(today + timedelta(days=1)).strftime('%Y-%m-%d'), interval="1d")

        if amzn.empty or sp500.empty or oil.empty or dollar_index.empty:
            return None, "Không có dữ liệu cho ngày đã chọn (thị trường có thể đóng cửa)."
...
def predict_next_day():
    try:
        ...
        update_chart(prediction, df['Close'].iloc[-1], next_date.strftime('%Y-%m-%d'), df['Date'].iloc[-1].strftime('%Y-%m-%d'),
